...
class WindowRenderInfo(object):
    ...
    def get_height_for_line(self, lineno, ypos=1):
        """
        Return the height of the given line.
        (The height that it would take, if this line became visible.)

        :param ypos: The position on this screen where the line will be
            displayed. The line_prefix can depend on the vertical position and
            can influence the line height.
        """
        if self.wrap_lines:
            return self.ui_content.get_height_for_line(
                lineno, self.window_width, self.window.get_line_prefix, ypos)
        else:
            return 1
        ...
        def copy_line(line, lineno, x, y, is_input=False):
            ...
            if is_input and get_line_prefix:
                prompt = to_formatted_text(
                    get_line_prefix(width, lineno, y, is_soft_wrap=False))
...
#            x = -horizontal_scroll  # TODO: scroll horiontally.
            ...
            for style, text in line:
                ...
                for c in text:
                    ...
                    if wrap_lines and x + char_width > width:
                        ...
                        if get_line_prefix:
                            prompt = to_formatted_text(
                                get_line_prefix(width, lineno, y, True))
    ...
    def _scroll_when_linewrapping(self, ui_content, width, height):
        ...
        def get_line_height(lineno):
            return ui_content.get_height_for_line(lineno, width, self.get_line_prefix)   # XXX: pass 0 for ypos
        ...
        # scroll to this line.
        if get_line_height(ui_content.cursor_position.y) > height - scroll_offsets_top:
    ...
    def _scroll_without_linewrapping(self, ui_content, width, height):
        ...
        if self.get_line_prefix:
            current_line_prefix_width = fragment_list_width(to_formatted_text(
                self.get_line_prefix(width, ui_content.cursor_position.y, 0, False)))
        else:
